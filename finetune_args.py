...
model_g.add_arg("ernie_config_path",         str,  os.path.join(os.getcwd(), 'model', 'ernie_config.json'),           "Path to the json file for ernie model config.")
model_g.add_arg("init_checkpoint",          str,  "./checkpoints/step_1033", "Init checkpoint to resume training from.")
model_g.add_arg("init_pretraining_params",  str,  os.path.join(os.getcwd(), 'model', 'params'),
...
train_g.add_arg("weight_decay",      float,  0.0,    "Weight decay rate for L2 regularizer.")
...
data_g.add_arg("train_set",           str, os.path.join(os.getcwd(), 'lcqmc', 'Fine-tuning.tsv'), "Path to training data.")
data_g.add_arg("test_set",            str, os.path.join(os.getcwd(), 'lcqmc', 'test.tsv'), "Path to test data.")
data_g.add_arg("dev_set",             str, os.path.join(os.getcwd(), 'lcqmc', 'dev.tsv'), "Path to validation data.")
...
run_type_g.add_arg("do_train",                     bool,   True,  "Whether to perform training.是否进行训练")
run_type_g.add_arg("do_val",                       bool,   False,  "Whether to perform evaluation on dev data set.是否对开发数据集执行评估")
run_type_g.add_arg("do_test",                      bool,   False,  "Whether to perform evaluation on test data set.是否对测试数据集进行评估")

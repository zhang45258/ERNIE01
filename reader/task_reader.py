...
class ClassifyReader(BaseReader):
    def _read_tsv(self, input_file, quotechar=None):
        """Reads a tab separated value file.读取tab分隔的值文件"""
        with open(input_file, "r" ,encoding='UTF-8') as f: #f打开文件的句柄
            reader = csv.reader(f, delimiter="\t", quotechar=quotechar) #读f；用"\t"做分隔符，水平制表符；quotechar引用符是引用符
            headers = next(reader)  #返回reader的下一行
            #print(headers)
            text_indices = [
            ...
            相比与list，tuple中的元素不可修改，在映射中可以当键使用。
            ...
            for line in reader:
                for index, text in enumerate(line):
                    if index in text_indices:
                        if self.for_cn:
                            line[index] = text.replace(' ', '')#如果中文，将空格去掉
                        else:
                            line[index] = text
                example = Example(*line)
        ...
        '''
        [Example(text_a='谁有狂三这张高清的', text_b='这张高清图，谁有'),
        Example(text_a='红米刷什么系统好', text_b='红米可以刷什么系统'),
        Example(text_a='这叫什么高跟鞋', text_b='这种高跟鞋叫什么呀')]
        '''
